function GeoVetPHMap(options) {
	...
	var mapProperties = {
		...
		zoom: this.define.defaultFilter.zoom,
		panControl: false,
		rotateControl: false,
	...
	var formatDate = function() {
		// format default date
		var defaultDate = (["Today", "Past 2 days", "Past 3 days", "Past 4 days", "Past 5 days", "Past 6 days", "Past week", "Past 2 weeks", "Past month"].includes(self.define.defaultFilter.date.selected))?
...
GeoVetPHMap.prototype = {
	_processFilterData: function(filter) {
		...
		if(location.name !== "" || animal.length !== 0 || disease.length !== 0 || date.from !== this.define.defaultFilter.date.from || date.to !== this.define.defaultFilter.date.to) {
